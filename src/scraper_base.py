...
from view_base import view_base, view_bash
import os
import threading
...
class scraper_base_with_saving(scraper_base):
    def __init__(self, view = view_bash(), **kwargs):
...
class scraper_novel_with_saving(scraper_base_with_saving):
    ...
    def run_with_saving(self):
        ...
        self.view.print("We are building index page list right now...")
        page_index_url_list = self.scrape_index_list(self.url)

        self.view.print("There are {} pages to list all chapter index.".format(len(page_index_url_list)))
        self.view.print("We are building chapter url list right now...")
        ...
        if len(page_index_url_list) < 12 or self.force_disbale_multi_thread:
            for i, page_index_url in enumerate(page_index_url_list):
                ...
                self.view.print("\t[{}/{}] This index page has {} chapters...".format(i+1, len(page_index_url_list), len(chapter_url_list_on_this_page)))
            ...
            class multi_thread_scrape_chapter_list(multi_thread_scrape):
                def __init__(self, max_thread_num, scraping_list, method, view_ops: view_base):
                ...
                def chunk_handle(self, chunk_start_index: int, chunk: list[str], scraped_records: dict[int, list[str]]):
                    ...
                    for ii, url in enumerate(chunk):
                        ...
                        self.view_ops.print("\t[{}/{}] This index page has {} chapters...".format(chunk_start_index + ii, len(self.scraping_list), len(scraped_records[chunk_start_index + ii])))
                    return index_list

            scrape_chapter_list_mt = multi_thread_scrape_chapter_list(self.max_thread_num, page_index_url_list, self.scrape_chapter_list, self.view)
            scrape_chapter_list_mt.run()
            chapter_url_list = scrape_chapter_list_mt.get_result()

        self.view.print("We have found {} chapters.".format(len(chapter_url_list)))
        self.view.print("We are scraping per chapter content...")

        self.view.new_bar(len(chapter_url_list))
        if len(chapter_url_list) < 12 or self.force_disbale_multi_thread:
            for i, chapter_url in enumerate(chapter_url_list):
                ...
                self.view.bar_update(1)
                self.view.bar_description("Scraping {} of {} chapters".format(i+1, len(chapter_url_list)))

        else: # we use multi thread to speed up

            class multi_thread_scrape_chapter(multi_thread_scrape):
                def __init__(self, max_thread_num, scraping_list, method, write_op, view_ops: view_base):
                ...
                def chunk_handle(self, chunk_start_index: int, chunk: list[str], scraped_records: dict[int, list[str]]):
                    self.view_ops.bar_update(len(chunk))
                    self.view_ops.bar_description("Scraping {} of {} chapters".format(chunk_start_index+len(chunk), len(self.scraping_list)))
                    for ii, url in enumerate(chunk):
                        if chunk_start_index + ii not in scraped_records:
                            self.view_ops.print("We have missed {} whose index is {}!". format(url, chunk_start_index+ii))
                            continue
                        self.write_op(ii, url, scraped_records[chunk_start_index + ii][0])
                    return []

            scrape_chapter_mt = multi_thread_scrape_chapter(self.max_thread_num, chapter_url_list, self.scrape_chatper, self.write_file_handle, self.view)
            scrape_chapter_mt.run()

        self.view.print("We have finished scaping this book.")
        self.view.print("It saves as {}/{}.".format(os.getcwd(), self.file_name))
